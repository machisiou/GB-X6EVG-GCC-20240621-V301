...
{	
	...
	if(AC_IN_L)//if (AC_IN_L)
    {
        ...
        //bRWSMBus(SMbusCh1, SMbusWW, Charger_Addr, CHGCmd_Option,  &BAT_OPTION ,  SMBus_NoPEC);
		SMBUS_RW_W(0,WriteWord,Charger_Addr,CHGCmd_Option,  &BAT_OPTION);
		//Disable_pd_adapter(); //Raymond will implement this function in PD_UPDATE
    ...
    {
		NV_DB20_SUPPORT(); //981004-221228-A
	}		
	...
	//BIT_CLR(POWER_FLAG3, ac_present);						
    BIT_CLR(POWER_FLAG10, scion_ac_in); //992022-140904-A
	//BIT_CLR(POWER_FLAG6, dual_ac_out); //EC RAM 0x206 bit2 //981004-201224-A	
    SCI_on_AC_in_cnt = 0; //992022-140904-A
	AC_boost_Status = 0; //981004-151008-A
	AC_boost_cnt1 = 0; //981004-151001-A
	AC_boost_cnt2 = 0; //981004-151001-A	
    ...
    BIT_CLR(POWER_FLAG6, sv_thrt_on); //981004-190329-A
	//981004-200923-A-S
	//BIT_CLR(EC_Flag2, pdacin_chgcur);	
	CHGCUR_STEP_cnt2 = 0;	
	...
	if (IS_BIT_SET(EC_Flag4, old_99w)) //981004-220510-A
	{
 	   //BAT_CHGCURR = 0x8002; //0.512+0.128 = 0.64A (Target : 0.637A )
       BAT_CHGCURR = 0x0001;
	}
    ...
    else
    {
 	...
 	//AC_PRESENT = 0;
    BIT_CLR(EC_Flag5, BAT_1C_CHG); //EC RAM 0x389 bit6 //981004-220811-A for 1C charge
    BIT_CLR(EC_Flag5, BAT_1C_CHG); //EC RAM 0x389 bit6 //981004-220419-A	
	FAST_CHG_CNT1 = 0; //981004-230925-A
	FAST_CHG_CNT2 = 0; //981004-230925-A
	CHG_TEMP1 = 0; //981004-230925-A
...
{   	
    ...
    {	    
        ...
        if (((GPIO_Read(16)) && (IS_BIT_CLR(POWER_FLAG11, pd_ac_over65w))) ||(GPIO_Read(113))) //battery mode	
		{		
		   ...
		   //   }			   
			   _Delay_1ms(250);		   
               patchACout = 0; 				   
               GPIO_Write(0,0);//EC_3A = 0;  			
	           GPIO_Write(99,0);//RSMRST_L = 0; //GPIOH0 GPIO099
				//5571 register
//               if (ECHIPVER >= _CHIP_EX) //981004-230425-A
//               {    
//                   SET_MASK(ESGCTRL2, F_ESPIIPG);
//               }   
		       GPIO_Write(122,0);//EC_EN_V5A = 0;
		   }	   
		}
		else
		{
		   GPIO_Write(122,1);//EC_EN_V5A = 1;		   
		   ...
		   if (IS_BIT_SET(POWER_FLAG12, bat_full) || (IS_BIT_SET(BAT_ALARM1, FullyChg))) //981004-181123-M
		   {
			  ...
			  {	
                ...
                #endif                
				_Delay_1ms(250);
			    GPIO_Write(0,0);//EC_3A = 0; //981004-141020-A  			
	            GPIO_Write(99,0);//RSMRST_L = 0; //981004-141020-A	
//                if (ECHIPVER >= _CHIP_EX) //981004-230425-A
//                {    
//                    SET_MASK(ESGCTRL2, F_ESPIIPG);
//                }       
