                                 ...
                                 size_t size_bits )
{
    ...
    if( remaining_bits > 0U )
    {
        ...
        if( remaining_bits > 0U )
        {
            ...
            unsigned char mask = (unsigned char)((uint64_t) (1U << remaining_bits) - 1U);
...
{
	//----------------LALU_SHA3: the finalize function is replaced by lalu sha3 ------
...
//    (void) mbedtls_keccak_f_permute( &ctx->keccak_f_ctx );
	...
	if(ctx->queue_len!=0)
	{
		lalu_sha3_update((DWORD)&ctx->queue,(ctx->queue_len/8U));
		lalu_sha3_final();
	}
	else
	{
                                  ...
                                  const mbedtls_keccak_sponge_context *src )
{
    *dst = *src;
	...
	//----------------------LALU_SHA3:Backup lalu_sha3 hash ---------------
	lalu_sha3_hash_out((DWORD)&dst->keccak_f_ctx);	// copy hash from lalu_sha3 to context

	#ifdef SHA3_DEBUG
		printf("mbedtls_keccak_sponge_clone >> & dst->keccak_f_ctx = 0x%x \n", &dst->keccak_f_ctx);
	#endif
	//---------------------------------------------------------------------
}
                                  ...
                                  size_t suffix_len  )
{
	...
	//-------------------------LALU_SHA3:setup lalu_sha3 config register----------
	unsigned int lalu_sha3_type=0;
	//type : SHA3
	switch(ctx->rate)
	{
		case 1344U:
			lalu_sha3_type = 0x000;
			break;

		case 1152U:
			lalu_sha3_type = 0x100;
			break;

		case 1088U:
			lalu_sha3_type = 0x200;
			break;

		case 832U:
			lalu_sha3_type = 0x300;
			break;

		case 576U:
			lalu_sha3_type = 0x400;
			break;

		default:
			#ifdef SHA3_DEBUG
				printf("error@keccak.c (lalu_sha3_setup) - no valid setting in ctx->rate or ctx->suffix_len \n");
			#endif
			break;
	}

	if(ctx->suffix_len!=2U)
	{
		lalu_sha3_type |= 0x1;
	}
	write_word(SHA3_SPEC_TYPE, lalu_sha3_type);
	//------------------------------------------------------------
    return( 0 );
}
    ...
    if( remaining_bytes > 0U )
    {
        ...
        if( ctx->queue_len > 0U )
        {
            ...
            if( remaining_bytes >= queue_free_bytes )
            {
				...
				//--------------------LALU_SHA3 : replace hash process with lalu sha3 ---------------
//                (void) mbedtls_keccak_f_xor_binary( &ctx->keccak_f_ctx,
//                                                    ctx->queue, ctx->rate );
//                (void) mbedtls_keccak_f_permute( &ctx->keccak_f_ctx );
				lalu_sha3_update((DWORD)&ctx->queue,rate_bytes);
				//-------------------------------------------------------------------------------
            }
            else
            {
...
//            (void) mbedtls_keccak_f_permute( &ctx->keccak_f_ctx );
			lalu_sha3_update((DWORD)&data[data_offset],rate_bytes);
			//---------------------------------------------------------------------------
                                   ...
                                   size_t size )
{
	...
	//---------------------LALU_SHA3 : declare variable for lalu_sha3 ---------------
	size_t rate_word = ctx->rate/32;
	size_t remain_word;
	size_t addr_data;
	size_t addr_sha3;
	//-------------------------------------------------------------------------------
    ...
    if( size > 0U )
    {
        ...
        {
        	//--------------LALU_SHA3 : replace hash process with lalu_sha3 ------
//            (void) mbedtls_keccak_f_permute( &ctx->keccak_f_ctx );
//            (void) mbedtls_keccak_f_read_binary( &ctx->keccak_f_ctx,
//                                                 &data[data_offset],
//                                                 rate_bytes );

			addr_data = (size_t)&data[data_offset];
			addr_sha3 = SHA3_HASH_BASE;
			remain_word = rate_word;

			lalu_sha3_squeeze();

			// write hash to dst address
			while(remain_word!=0)
			{
				write_word(addr_data, read_word(addr_sha3));
				addr_data += 4;
				addr_sha3 += 4;
				remain_word--;
			}
